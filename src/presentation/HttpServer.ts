...
export class HttpServer {
  ...
  private ensureLocalCapabilitySubscription(): void {
    ...
    const capabilitySyncManager = this.agent.getCapabilitySyncManager();
    if (!capabilitySyncManager) {
      this.logger.warn(
        "Cannot subscribe to local capabilities: CapabilitySyncManager not initialized"
      );
      return;
    }
